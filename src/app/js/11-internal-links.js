...
async function generateArticle(){
  ...
  try{
    ...
    const csvBlock=linkPool.length?`\n\nINTERNAL LINKS — CANDIDATES (pick 2-4 that genuinely fit THIS article; skip the rest):\n`+linkPool.map(r=>`${r.url} — ${r.title}`).join('\n'):'';
    ...
    const ideasBlock=`\n\nIDEAS ARTICLE: an editorial round-up for "${kw}" (${v('category')}, ${v('audience')}) with EXACTLY ${target} ideas. `
      ...
      + `Use only 2-3 of the available blocks on any given idea — pick whichever genuinely apply to THAT theme, and vary which ones you use from idea to idea so no two cards read the same. If an idea has nothing specific worth putting in a card, output "planner": [] and skip it; roughly one idea in five should have no card at all. Never pad a card to fill the shape. `
      + `EVERY idea also carries its own "shop" list of 2-4 Amazon search phrases chosen for that idea, and "extraImagePrompts" with 0-2 extra photo prompts used only when a genuinely different shot is worth showing (the cake, the dessert table, a decor close-up). Do not put "shop" on the sections. Write it all like a party planner selling the idea to the mum who will host it.`;
    ...
    const msg=`Write a complete ${label} article.\n\nMain keyword: ${kw}\nCategory: ${v('category')}\nAudience: ${v('audience')}\nContext: ${brief||'(none)'}${titleBlock}${modeBlock}${csvBlock}${paaBlock}${rel}${noFiller}${seasonBlock}\n\nReturn the JSON only.`;
