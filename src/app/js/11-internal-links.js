    ...
    const ideasBlock=`\n\nIDEAS ARTICLE: an editorial round-up for "${kw}" (${v('category')}, ${v('audience')}) with EXACTLY ${target} ideas. `
      ...
      + (wantDl
          ...
          : `THIS IS A PHOTO IDEAS ARTICLE: EVERY entry is asset "illustration", a real editorial photograph of the idea itself styled in its setting (the cake on a dressed table, the balloon arch in a decorated room), never a printable sheet, never flat vector art, and with NO text rendered on the image. `
            + `WRITE EACH imagePrompt AS A REAL SCENE FOR "${kw}", 3 to 5 sentences, not a product listing. Say what the thing is made of and what it stands on, name the surface and the room it lives in, and place two or three other objects around it that belong to THIS occasion and would honestly be there. Colours, materials and props all come from the "${kw}" theme, so a reader could tell which article the photograph belongs to with the caption covered. Never write "product photography", "product styling", "studio", "on a white background" or "isolated on": this is a room someone decorated, not a catalogue. `)
