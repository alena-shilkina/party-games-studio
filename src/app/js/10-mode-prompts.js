...
function syncRecipeCards(art,mode){
  ...
  (art.games||[]).forEach(g=>{
    ...
    /* Обрезать подписи кодом пробовали и отказались: «2 tbsp butterfly pea flowers»
       превращалось в «2 tbsp butterfly», то есть в другой ингредиент. Осмысленно
       сократить может только модель, поэтому короткая подпись требуется в схеме,
       а здесь остаётся лишь снять лишние пробелы. */
    ing=ing.map(x=>String(x).replace(/\s+/g,' ').trim());
    ...
    // Панель совета появляется только когда совет настоящий: выдуманный «pro tip»
    // на референсах и есть та вода, которой мы не хотим.
    const tipText=String(card.tip||'').trim();
    const clean=t=>String(t).replace(/["']/g,"'");
    const tipZone=tipText
      ? ` (6) TIP BOX: a small accent panel set apart from the others, narrower than the panels above it and in a contrasting tint, tucked against one side rather than spanning the full width, with a tiny painted icon and a label chip reading TIP. It carries this one line and nothing else, copied word for word: [${clean(tipText)}]. Invent no second tip and no extra sentence.`
      : ` There is no tip box on this card. Do not add one, and do not invent a tip, a note, a rating, a hashtag or a handle to fill space.`;
...
(0) TWO DIFFERENT MEDIA SIT ON THIS ONE CARD, and keeping them apart is the single most important thing here. The hero at the top is a REAL PHOTOGRAPH taken with a camera, and it must look like a photograph pasted into a designed layout. Everything else, the ingredient items, the step tiles, the ornament, is HAND-PAINTED illustration. Do not let the painted style bleed into the photograph, and do not let the photograph turn into a 3D render or a glossy product shot. If the finished card reads as one uniform illustration from top to bottom, it is wrong: the seam between the photograph and the painted layout should be visible and deliberate, the way a magazine pastes a real photo onto an illustrated page.
...
(4) INGREDIENTS PANEL: a filled rounded panel with a label chip reading WHAT YOU NEED, holding the ingredients as SMALL HAND-PAINTED ILLUSTRATIONS (not a text list): each item drawn on its own, with its name and amount in small clean type underneath. Show EXACTLY these ${ing.length}, in this order, copied word for word: `
      ...
      + tipZone
      + ` LAYOUT, AND THIS IS WHERE MOST CARDS GO FLAT: do not stack every block as one identical full-width band down the page. Vary the shapes the way a designed magazine page does. Panels may differ in width and in corner treatment, a narrower panel may sit beside a wider one where the content is short, a label chip may straddle a panel edge rather than sit inside it, and a small painted motif may overlap a panel corner or the frame of the photograph. Give the badge row, the ingredients and the steps three visibly DIFFERENT container shapes rather than three copies of the same rounded rectangle. The reader's eye should have somewhere to travel.
TYPOGRAPHY carries as much of the design as the panels: the recipe name is a display face with real personality, the label chips are small confident capitals with letter-spacing, the item names are a clean readable face, and one accent line may be a script. Three faces at most, used consistently.
...
CRITICAL: every word above must be rendered exactly as given, spelled correctly and large enough to read comfortably at arm's length. LEGIBILITY BEATS FITTING MORE IN: an ingredient label is at most three short words on ONE line, never wrapped onto two, never squeezed, never overlapping its neighbour or its own illustration. If a label will not fit at a comfortable size, make the illustration smaller and the type bigger, never the other way round. A doubled, smeared or half-formed word is a ruined card. The article text carries the full precise recipe, so the card shows only what is listed here.`;
    synced++;
  });
