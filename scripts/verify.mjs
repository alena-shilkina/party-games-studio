...
group('Правила человеческого текста');
{
  ...
  // Фото уезжало от своей идеи: под заголовком про навес выходили шары и ящик овощей.
  js.includes('SUBJECT OF THIS PHOTOGRAPH:')
    ? ok('тема кадра задаётся названием идеи') : fail('тему кадра снова решает только модель');
  js.includes('The subject is the one named at the top of this prompt')
    ? ok('хвост ссылается на заданную тему') : fail('хвост снова отсылает к абзацу, а не к теме');
  js.includes('фото не про свою идею')
    ? ok('расхождение фото и идеи попадает в замечания') : fail('расхождение фото и идеи не видно');

  // Планы раздавались по кругу вслепую, и занавесу на карнизе достался вид строго
  // сверху: генератор разложил его на полу.
  /const HANGS_ON_WALL=/.test(js) && /function framingsFor\(/.test(js)
    ? ok('план подбирается под предмет') : fail('план снова раздаётся вслепую');
  js.includes('THE SUBJECT OUTRANKS THE FRAMING, ALWAYS')
    ? ok('предмет объявлен важнее плана') : fail('невозможный план некому отменить');
  js.includes('laying a hanging decoration down')
    ? ok('раскладывать висящее запрещено') : fail('висящее снова разложат на полу');
