...
group('Фотографии');
{
  ...
  js.includes('WHAT TO CUT WHEN YOU RUN OVER')
    ? ok('при перерасходе режутся рассуждения, а не факты') : fail('модель срежет практические детали');
  ...
  js.includes('NEVER the practical detail')
    ? ok('практические детали при сокращении неприкосновенны') : fail('модель срежет цифры вместо рассуждений');
  js.includes('')
