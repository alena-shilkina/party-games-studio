...
group('Правила человеческого текста');
{
  ...
  // Поле Tone сохраняется в браузере и стоит ВЫШЕ голоса: старый сохранённый текст
  // задавал интонацию вместо него, и голос было не слышно.
  js.includes('function toneText(')
    ? ok('старый сохранённый тон не перебивает голос') : fail('сохранённый тон снова главнее голоса');
  (js.split('const tone=toneText();').length-1)===4
    ? ok('во всех 4 режимах тон берётся через toneText') : fail('где-то остался прямой v(tone)');
...
group('Печатные листы');
{
  ...
  // Ротации ракурсов больше нет вовсе, но «места на празднике» не должны вернуться:
  // когда-то этот список подменял кадр сюжетом и уводил фото от самой идеи.
  !js.includes('SHOT_TYPES')
    ? ok('список «мест на празднике» не вернулся') : fail('вернулся список «мест на празднике» вместо кадра идеи');
...
group('Фотографии');
{
  /* Ротация ракурсов и света убрана целиком: восемь планов с диафрагмами давали
     случайные и порой невозможные кадры (висящий занавес снимали сверху), а простой
     предмет получал крупный каталожный план. Осталась одна строка про реалистичное
     фото при естественном свете, ракурс решает генератор. */
  ['SHOT_FRAMINGS', 'SHOT_LIGHT', 'f/2.8', 'f/8'].forEach(s =>
    js.includes(s) ? fail(`ротация вернулась: ${s}`) : ok(`${s} в промптах фото нет`));
  js.includes('Realistic editorial photograph in natural daylight')
    ? ok('фото описано одной короткой строкой') : fail('короткой строки про фото нет');
  js.includes('Not a product shot, not a catalogue, not a studio')
  ...
  // Потолок длины ставится ТОЛЬКО Клоду: у моделей Runware беда обратная,
  // текст куцый, и рамка сверху сделала бы только хуже.
  js.includes("const lengthBlock=(textModel()==='claude')")
    ? ok('потолок длины только для Клода') : fail('потолок снова достаётся всем моделям');
  js.includes('a ceiling not to sail past')
    ? ok('это потолок, а не цель') : fail('потолка нет');
  !js.includes('WHAT TO CUT WHEN YOU RUN OVER')
    ? ok('порядок сокращения убран, текст не высушится') : fail('модель снова срежет рассуждения');
  !js.includes('PER ENTRY: every entry gets')
    ? ok('нормы на пункт нет, слабые модели не зажаты') : fail('норма на пункт зажмёт слабую модель');

  /* Вёрстка под рекламу: блоки Mediavine встают МЕЖДУ элементами верхнего уровня,
     поэтому доход зависит от числа границ, а не от объёма. Правила были только
     в режимах игр и карточек, а в идеях и рецептах их не было вовсе. */
  js.includes('const AD_LAYOUT=')
    ? ok('правила вёрстки под рекламу есть') : fail('правил вёрстки под рекламу нет');
  (js.split('${AD_LAYOUT}').length - 1) === 4
    ? ok('вёрстка под рекламу во всех 4 режимах') : fail('где-то режим остался без правил вёрстки');
  js.includes('NUMBER OF BOUNDARIES in the HTML')
    ? ok('названа настоящая причина: границы блоков') : fail('правило снова про объём, а не про границы');
  js.includes('')
...
group('Доставка правил до модели');
{
  /* Runware не документирует, что делает с ролью system у каждой модели. Если она
     отбрасывается, до модели не доходит НИ ОДНО правило, и статья выглядит так,
     будто промпта не было. Поэтому шлём одним сообщением. */
  js.includes("let messages=[{role:'user',content:String(system||'')")
    ? ok('правила и задание уходят одним сообщением') : fail('роль system снова может быть отброшена');
  !js.includes("messages=[{role:'system'")
    ? ok('отдельной роли system нет') : fail('роль system вернулась');
}
